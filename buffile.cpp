...
int BufferFile::Open (char * filename, ios::openmode mode)
...
// use ios::nocreate to ensure that a file exists
{
    ...
    //Jie: noreplace is not allowed. Removed to run the program !!!
	
	//File . open (filename, mode|ios::in|ios::nocreate|ios::bin);
	//Jie: nocreate is not allowed. Removed to run the program!!!
 ...
 // write the current buffer contents
{
	if (recaddr == -1)
	    ...
	    //call Write() from IOBuffer but Write() is abstract virtual function
		//thus it needs the definition in the drived class
	else
		return Buffer . DWrite (File, recaddr); //call from IOBuffer
}
