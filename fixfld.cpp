...
static const char * headerStrF = "Field";
static const int headerStrSizeF = strlen (headerStrF);
...
{
	char * str = new char[headerStrSizeF+1];
	...
	stream . read (str, headerStrSizeF);
	if (!stream.good()) return -1;
	if (strncmp (str, headerStrF, headerStrSizeF) != 0) return -1;
...
//	field sizes			4 bytes per field
{
	...
	stream . write (headerStrF, headerStrSizeF);
	if (!stream.good()) return -1;
